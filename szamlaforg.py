...
#
# Történet:
#  2020.12.01: könyvelt egyenleg mező kimaradt 2020.02.01-től kezdődően
#  2019.05.05: fájlok sorban importálódnak, commit csak a végén, sqlitebrowser indítása a végén
...
class szamla:
  ...
  def load_file(self,fajlnev):
    ...
    valtozas_datuma = datetime.date(2020,1,31)
    for row in rows:
        cols = row.find_all('td')
        # kiveszi a \xa0 karaktereket
        cols = [unicodedata.normalize("NFKD", ele.text.strip()) for ele in cols]
        ccols=cols[3].split(".")
        tranzakcidatum = datetime.date(int(ccols[0]),int(ccols[1]),int(ccols[2]))
        if tranzakcidatum > valtozas_datuma: # 2020.02.01 után nincs könyvelt egyenleg
          cols.insert(5,0)
        data.append(cols)
    return head,data
#-----------------------------
